...
export class DatasetComponent implements AfterViewInit, OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.tableId&&!this.isCollab)
      this.collab.GetCollabsForTable(this.tableId).subscribe(next=>this.collaborators=next);
  }

  deleteSheet() {
    console.log(this.tableId);
    this.user
      .DeleteDataSet(this.tableId!)
  ...
  onEditComplete(event: {field:string, data:any, originalEvent:Event,index:number}): void {
    ...
    this.apollo.lookUpDataSetId(this.tableId).subscribe((id: string) => {
      //Get the GraphqlId
